...
def maxSubArray(nums):
    ...
    for n in nums:
        if current_sum< 0: #if the element is negative, we reset and start fresh from the next element.
            current_sum=0  #we reset and start fresh from the next element.
        current_sum+=n
